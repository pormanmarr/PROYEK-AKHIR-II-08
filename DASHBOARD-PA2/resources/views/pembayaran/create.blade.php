...
<div class="row">
    <div class="col-md-8">
        <h2><i class="bi bi-plus-circle"></i> Tambah Pembayaran</h2>
    </div>
</div>
        ...
        <form action="{{ route('pembayaran.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('jumlah_bayar') is-invalid @enderror" 
                       id="jumlah_bayar" name="jumlah_bayar" value="{{ old('jumlah_bayar') }}" step="0.01" required>
                @error('jumlah_bayar')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('tgl_pembayaran') is-invalid @enderror" 
                       id="tgl_pembayaran" name="tgl_pembayaran" value="{{ old('tgl_pembayaran', date('Y-m-d')) }}" required>
            ...
            <div class="mb-3">
                ...
                <select class="form-control @error('status_bayar') is-invalid @enderror" id="status_bayar" name="status_bayar" required>
                    ...
                    <option value="diterima" {{ old('status_bayar') == 'diterima' ? 'selected' : '' }}>Diterima</option>
                    <option value="ditolak" {{ old('status_bayar') == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                </select>
                @error('status_bayar')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
